"""
=============================================================================
 DIAGRAMA DE CARACTERÍSTICA TRACTIVA -- APP WEB (Streamlit)
 Autor: Pablo | Ingeniería en Sistemas Automotrices

 Versión "pro" del script de Python: formularios en vez de captura por
 consola, gráfica interactiva (zoom, hover, exportar imagen), puntos
 óptimos de cambio de marcha, curva de aceleración vs velocidad, y
 guardar/cargar la configuración capturada en un archivo JSON.

 Cómo correrla:
   pip install streamlit plotly pandas numpy scipy openpyxl kaleido
   streamlit run app_tractiva.py

 Eso abre automáticamente tu navegador en algo como http://localhost:8501
 Ahí ya no se ve nada de código -- solo la página con el formulario y la
 gráfica, como una app de verdad.
=============================================================================
"""
...
# ---------------------------------------------------------------------------
# CONFIGURACIÓN DE PÁGINA + ESTILO (tema oscuro, rojo Ducati)
# ---------------------------------------------------------------------------
st.set_page_config(
...
st.title("🏍️ Diagrama de Característica Tractiva")
...
# ---------------------------------------------------------------------------
# BARRA LATERAL: CONFIGURACIÓN GENERAL + CARGAR CONFIGURACIÓN GUARDADA
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PANTALLA INICIAL: PREGUNTA OBLIGATORIA ANTES DE MOSTRAR EL FORMULARIO
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FORMULARIOS POR MODELO (en columnas o tabs)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GUARDAR CONFIGURACIÓN ACTUAL (JSON) -- disponible sin necesidad de generar
# ---------------------------------------------------------------------------
config_export = {
    "n_modelos": n_modelos,
...
if generar:
    ...
    # -----------------------------------------------------------------
    # GRÁFICA INTERACTIVA DE TRACCIÓN (Plotly, estilo Ducati)
    # -----------------------------------------------------------------
    fig = go.Figure()
    ...
    c1, c2, c3 = st.columns(3)
    c1.download_button("⬇️ Descargar tabla (Excel)", data=excel_buffer.getvalue(),
                        file_name=f"Resultados_tractiva_{fecha}.xlsx", use_container_width=True)
    if png_bytes is not None:
        c2.download_button("⬇️ Descargar tracción (PNG)", data=png_bytes,
                            file_name=f"Curva_traccion_{fecha}.png", use_container_width=True)
    if png_bytes_acc is not None:
        c3.download_button("⬇️ Descargar aceleración (PNG)", data=png_bytes_acc,
